...
import { onSnapshot, getDocs, doc, collection, query, where, deleteDoc, addDoc } from 'firebase/firestore';
...
const FavoritesScreen = () => {
	...
	useEffect(() => {
		const FetchFavorites = async () => {
			...
			const favoritesQuery = query(collection(db, 'favorites'), where('userId', '==', currentUserId));
			...
			onSnapshot(favoritesQuery, (snapshot) => {
				...
				if (productIds.length > 0) {
					const productQuery = query(collection(db, 'products'), where('productId', 'in', productIds));
