...
const EditProfileScreen = () => {
	...
	const isEditingEnabled = !(status !== 'Unverified' && status !== 'Rejected');
			...
			try {
				...
				if (userDoc.exists()) {
					...
					if (data.profileImage) {
						// Set the selected image to the profile image URL
			...
			<View style={styles.upperContainer}>
				...
				<View style={styles.pic_cont}>
					{selectedImage ? (
						...
						<Image source={DefaultImage} style={styles.pic_cont} className="w-full h-full rounded-full" />
					)}
				</View>

				<TouchableOpacity onPress={isEditingEnabled ? pickImageAsync : null}>
					{isEditingEnabled && <IconSimple style={styles.camera} name="camera" size={16} color="white" />}
				</TouchableOpacity>

				<View style={styles.statusView}>
					<Text style={styles.statusText}>{status ? status : 'Unverified'}</Text>
				</View>
			</View>
				...
				<View style={styles.labelInfoCont}>
					...
					<View style={styles.infoContView}>
						<Picker
							...
							onValueChange={(itemValue) => setUpdateUserData({ ...updateUserData, gender: itemValue })}
